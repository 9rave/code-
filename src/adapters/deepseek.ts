// DeepSeek 适配器（MVP4 预留，见开发指南 §4.1 / §16）
// OpenAI 兼容 Chat Completions 接口；仅启用 AI 且配置了 DEEPSEEK_API_KEY 时生效。
...
export class DeepSeekAdapter implements ModelAdapter {
  ...
  async healthCheck(): Promise<ProviderHealth> {
    return { ok: !!this.key, detail: this.key ? undefined : "DEEPSEEK_API_KEY 未配置" };
  }
  ...
  async generateReview(input: ReviewInput, opts: GenerateOptions): Promise<ModelResult> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`DEEPSEEK_HTTP_${res.status}: ${detail.slice(0, 200)}`);
    }
