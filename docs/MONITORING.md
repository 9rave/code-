...
> 2. **业务派生告警** —— 数据已落 D1（`push_logs` / `ai_usage` / `rate_limits` / `users`），用 `scripts/healthcheck.mjs` 周期轮询，可接 Cloudflare Alerting Webhook 或企微机器人（§2–§3）。
>
...
路径：**Workers & Pages → ai-todo → Settings → Metrics & Analytics / Notifications**，或 **Notifications（铃铛）→ Add notification**。以下规则建议全部开启，通知渠道选「Email」+「Webhook」（Webhook 可转发到企微机器人）。
...
| P5 | 子请求失败 | `worker_subrequest` failed | > 0.5%（5 分钟） | Medium | 多为企微 webhook / Workers AI 网络问题 |

**通知渠道建议**：告警统一发到**独立**的运维企微群（不要和业务复盘推送混在同一群），或用 Cloudflare Email + PagerDuty（生产必配 on-call）。
...
| # | 指标 | 数据源 | 判定 | 严重度 |
|---|---|---|---|---|
| B1 | 企微推送失败 | `push_logs.status='failed'`（当日+前一日） | 任意失败 → **High** | 业务核心，用户收不到晨/夕/周复盘 |
